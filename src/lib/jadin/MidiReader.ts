	...
	readEvent(): RawEvent {
		...
		if (firstByte == 0xff) {
			...
			switch (subtypeByte) {
				case 0x00:
					if (length != 2) {
						throw 'Length for this sequenceNumber event was ' + length + ', but must be 2';
					}
				...
				case 0x2f:
					if (length !== 0)
						throw new Error('Length for this endOfTrack event was ' + length + ', but must be 0');
				...
				case 0x51:
					if (length !== 3)
						throw new Error(`Length for this setTempo event was ${length}, but must be 3`);
				...
				case 0x54:
					if (length !== 5)
						throw new Error(`Length for this smpteOffset event was ${length}, but must be 5`);
