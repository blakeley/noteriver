<script lang="ts">
	...
	import PianoScene from './PianoScene.svelte';
	import * as THREE from 'three';

	// Light control state
	let lightX = $state(0);
	let lightY = $state(-100);
...
<div class="flex h-full w-full flex-col items-center justify-center gap-4 bg-neutral-200">
	<h1 class="text-2xl font-bold">3D Piano Keyboard Demo</h1>
	...
	<div class="aspect-video w-full max-w-screen-lg border-4 border-dashed border-gray-500">
		<Canvas toneMapping={THREE.NoToneMapping}>
			<PianoScene {lightX} {lightY} {lightZ} {lightIntensity} />
		</Canvas>
	</div>
</div>
